...
def args():
    ...
    parser.add_argument("-l", "--liste", help="consensus list.", type=str, required=True)
    ...
    return args.cv, args.x, args.y, args.folds, ROWS, COLS, args.liste
...
def main():
    cv_dir, x, y, nbfolds, ROWS, COLS, list_25 = args()
    ...
    for i in range(nbfolds):
        ...
        for suffix in ["test"]:
            ...
            pdb_list = np.loadtxt(os.path.join(path_CV, f"{suffix}_pdb_id.txt"), dtype="str")
            pdb_list_25 = np.loadtxt(list_25, dtype="str")


            pdb_test_25 = []

            for pdb_id in pdb_list:
                if pdb_id in pdb_list_25:
                    pdb_test_25.append(pdb_id)

            pdb_list = pdb_test_25
            print(len(pdb_test_25))
            ...
            with h5py.File(os.path.join(path_CV, f"Y_{suffix}_evaluate25.h5"), "w") as f_Y:
                dset = f_Y.create_dataset(f"y_{suffix}", data = Y)


            #Create X files
            bar = IncrementalBar("Generating X files", max = len(pdb_list))

            with h5py.File(os.path.join(path_CV, f"X_{suffix}_evaluate25.h5"), "w") as f_X:
                dset = f_X.create_dataset(f"x_{suffix}", (N, ROWS, COLS), dtype = np.float32)
